...
async def send_help_img(bot: Bot, ev: Event):
    ...
    await bot.send("现在开始算符小游戏，你有60秒的时间回答问题，只需要回答数字即可，你可以发送取消来中止游戏。")
        ...
        async with timeout(60):
            while True:
                ...
                if resp is not None:
                    ...
                    if not result:
                        await bot.send(f"回答错误，正确符数为 {mahjong_scoring.answer_in_number}")
                        await bot.send(await convert_img(await mahjong_scoring.draw_problem_image(include_answer=True)))
                    else:
                        await bot.send("回答正确！")
